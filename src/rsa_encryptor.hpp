...
namespace RSAImpl {
    ...
    void RSAEncryptor::encrypt(std::istream& input, std::ostream& output) const
    {
        ...
        int blockSize = _size / 16 + (_size % 16 != 0 ? 1 : 0);
        ...
        for (int i = 0; i < blocks; ++i)
        {
            ...
            LongInt encryptedBlockLongInt = encrypt(blockLongInt).setSize(16 * blockSize);
            Byte* encryptedBlock = encryptedBlockLongInt.toBytes();

            output.write((char*) encryptedBlock, 2 * blockSize);
        }
    }
    ...
    void RSAEncryptor::decrypt(std::istream& input, std::ostream& output) const
    {
        ...
        int blockSize = _size / 16 + (_size % 16 != 0 ? 1 : 0);
        int blocks = (inputSize - sizeof(int)) / (2 * blockSize);
        int diff = 0;
        ...
        {
            Byte* block = new Byte[blockSize];
            input.read((char*) block, 2 * blockSize);

            LongInt blockLongInt = LongInt::fromBytes(block, 16 * blockSize);
